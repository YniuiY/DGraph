...
void ThreadPool::threadFunction(function<void()>&& func) {
  ...
  for (;;) {
    if (isShutdown) {
      ...
      if (!coreThread && status == cv_status::timeout &&
          (getTaskQueueSize() < taskQueueLength)) {
        cout << "\n*** none core thread exit ***" << endl;
        livingThread--;
        break;
      } else if (coreThread && status == cv_status::timeout &&
                 getTaskQueueSize() == 0) {
...
int ThreadPool::getTaskQueueSize() { return (int)taskQueue.size(); }