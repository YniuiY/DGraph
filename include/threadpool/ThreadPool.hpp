...
enum Unit { Hours, Minutes, Second, Millisecond, Microsecond, Nanosecond };
...
class ThreadPool {
 private:
  ...
  std::queue<std::function<void()>>
      taskQueue;  // function模板可以将函数、指针、lambda等多种可调用对象，描述成一种可调用对象
  ...
  void threadFunction(
      std::function<
          void()>&&);  // 线程函数，起来以后是独立线程，任务函数执行在这个函数中
 ...
 public:
  ...
  ThreadPool(int maxCount, int coreCount, int tQueueLength, Policy p, int lTime,
             Unit u);
  ~ThreadPool();
  ...
  bool execute(Func&& task, Args&&... args) {
    bool ret = true;
    if (livingThread >= coreThreadCount &&
        getCurrentTaskQueueSize() < taskQueueLength) {
      ...
      cout << "new thread \n";
      noneCoreThreadQueue.push_back(
          thread(&ThreadPool::threadFunction, this, bind(task, args...)));
      livingThread++;
    } else if (livingThread >= maxThreadCount &&
               getCurrentTaskQueueSize() >= taskQueueLength) {
      // reject，队列满，线程数也到达最大线程数，这个时候调用拒绝策略
      cout << "reject task \n";
      RejectPolicyFactory::getInstance()->getRejectPolicy(policy)->reject(
          bind(task, args...));
      ret = false;
    }
    return ret;
